...
One extra step you'll need to take after you install Rust, but before you can use Hira is to add the `wasm32-unknown-unknown` target via `rustup`. Hira needs to compile certain parts of your code into Webassembly during compile time, and as such it needs this target to be available:

```sh
rustup target add wasm32-unknown-unknown
```

Now you're ready to start using Hira. Start by adding Hira to the dependencies of your Cargo.toml file.
...
<!-- TODO: add link to section explaining how to use hira with cargo build -->
